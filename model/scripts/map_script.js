  ...
  fetch("controllers/get_dashboard_data.php")
    ...
    .then((data) => {
      ...
      if (data.stats) {
        ...
        document.getElementById("total-illness").textContent =
          data.stats.total_illness_records || "0";
        document.getElementById("total-citizens").textContent =
          data.stats.total_citizens || "0";
...
function processClusterData(mapData) {
  ...
  Object.keys(clusterData).forEach((cId) => {
    clusterData[cId].total = Object.values(clusterData[cId].diseases).reduce(
      (a, b) => a + b,
      0
    );
  });
...
function initEnhancedMap(purokData) {
  ...
  purokData.forEach((item) => {
    ...
    const total =
      Number(item.dengue || 0) +
      Number(item.measles || 0) +
      Number(item.flu || 0) +
      Number(item.allergies || 0) +
    ...
    // Determine severity
    const severity =
      item.severity || (total > 50 ? "high" : total > 20 ? "medium" : "low");
    const severityColor =
      severity === "high"
        ? "#fee;color:#c00"
        : severity === "medium"
    ...
    const dominantDisease = Object.entries(diseases).reduce((a, b) =>
      a[1] > b[1] ? a : b
    )[0];
    ...
    const popupContent = `
        <div style="min-width:240px;">
          ...
          <table style="width:100%;border-collapse:collapse;margin-top:10px;">
            <tr style="border-bottom:1px solid #eee;"><td style="padding:6px 8px;color:#666;font-size:13px;">Dengue</td><td style="padding:6px 8px;text-align:right;font-weight:600;font-size:13px;">${
              item.dengue || 0
            }</td></tr>
            <tr style="border-bottom:1px solid #eee;"><td style="padding:6px 8px;color:#666;font-size:13px;">Measles</td><td style="padding:6px 8px;text-align:right;font-weight:600;font-size:13px;">${
              item.measles || 0
            }</td></tr>
            <tr style="border-bottom:1px solid #eee;"><td style="padding:6px 8px;color:#666;font-size:13px;">Flu</td><td style="padding:6px 8px;text-align:right;font-weight:600;font-size:13px;">${
              item.flu || 0
            }</td></tr>
            <tr style="border-bottom:1px solid #eee;"><td style="padding:6px 8px;color:#666;font-size:13px;">Allergies</td><td style="padding:6px 8px;text-align:right;font-weight:600;font-size:13px;">${
              item.allergies || 0
            }</td></tr>
            <tr style="border-bottom:1px solid #eee;"><td style="padding:6px 8px;color:#666;font-size:13px;">Diarrhea</td><td style="padding:6px 8px;text-align:right;font-weight:600;font-size:13px;">${
              item.diarrhea || 0
            }</td></tr>
...
function addClusterLegendToMap() {
  ...
  legend.onAdd = function () {
    ...
    sortedClusters.forEach((clusterId) => {
      ...
      const label =
        clusterId == -1 ? "Outliers" : `Cluster ${parseInt(clusterId) + 1}`;
      ...
      html += `
          <div style="display:flex;align-items:center;margin:8px 0;cursor:pointer;" 
            ...
            <div style="flex:1;min-width:0;">
              ...
              <div style="font-size:10px;color:#999;">${data.puroks.join(
                ", "
              )}</div>
            </div>
          </div>
...
function showError() {
  ["total-users", "total-illness", "total-citizens", "total-pending"].forEach(
    (id) => {
...
function refreshClusters() {
  if (
    !confirm(
      "This will refresh cluster data and may take a few seconds. Continue?"
    )
  )
    return;
